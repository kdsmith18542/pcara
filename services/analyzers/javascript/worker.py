...
class JavaScriptAnalysisWorker:
    def __init__(self):
    ...
    def run(self):
        """Main worker loop"""
        logger.info("JavaScript analyzer worker starting...")
    ...
    def process_job(self, job_info):
        ...
        if not any(lang in languages for lang in ["javascript", "typescript", "js", "ts"]):
            logger.info(f"Job {job_id} does not require JavaScript/TypeScript analysis, skipping")
            return
...
if __name__ == "__main__":
    worker = JavaScriptAnalysisWorker()
    worker.run()